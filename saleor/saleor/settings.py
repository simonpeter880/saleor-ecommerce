...
ENABLE_NPLUSONE_DETECTION = get_bool_from_env("ENABLE_NPLUSONE_DETECTION", DEBUG)
if ENABLE_NPLUSONE_DETECTION:
    try:
        import nplusone.ext.django  # noqa: F401

        MIDDLEWARE.append("nplusone.ext.django.NPlusOneMiddleware")
    except ImportError:
        pass  # nplusone not installed, skip
...
if ENABLE_NPLUSONE_DETECTION:
    ...
    NPLUSONE_WHITELIST = [
        {"label": "nplusone", "model": "contenttypes.ContentType"}
    ]
...
# Set maximum query depth to prevent deeply nested queries
# Set GRAPHQL_QUERY_MAX_DEPTH=0 to disable (not recommended)
...
CACHES["default"]["KEY_PREFIX"] = os.environ.get("CACHE_KEY_PREFIX", "saleor")
CACHES["default"]["VERSION"] = int(os.environ.get("CACHE_VERSION", "1"))
