...
class CategoryByIdLoader(DataLoader[int, Category]):
    ...
    def batch_load(self, keys):
        categories = (
            Category.objects.using(self.database_connection_name)
            .defer("description_plaintext", "background_image", "background_image_alt")
            .in_bulk(keys)
        )
...
class ProductChannelListingByProductIdAndChannelSlugLoader(
    ...
    def batch_load_channel(
        ...
        # First, get the channel ID from slug to build cache keys
        from ....channel.models import Channel

        try:
            channel = Channel.objects.using(self.database_connection_name).get(
                slug=channel_slug
            )
            channel_id = channel.id
        except Channel.DoesNotExist:
            # If channel doesn't exist, return None for all products
        ...
        if uncached_product_ids:
            product_channel_listings = ProductChannelListing.objects.using(
                self.database_connection_name
            ).filter(channel_id=channel_id, product_id__in=uncached_product_ids)
