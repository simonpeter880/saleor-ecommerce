...
from ...discount.models import Promotion as PromotionModel
...
class DiscountQueries(graphene.ObjectType):
    ...
    flash_sales = FilterConnectionField(
        PromotionCountableConnection,
        channel=graphene.String(
            required=True,
            description="Slug of a channel for which the flash sales should be returned.",
        ),
        description="List of active flash sales. No authentication required.",
        doc_category=DOC_CATEGORY_DISCOUNTS,
    )

    @staticmethod
    def resolve_flash_sales(_root, info: ResolveInfo, *, channel, **kwargs):
        """Resolve active flash sales for a channel (public, no auth required)."""
        qs = PromotionModel.objects.flash_sales().filter(
            rules__channels__slug=channel
