...
class ShippingMethodChannelListingByChannelSlugLoader(DataLoader):
    ...
    def batch_load(self, keys):
        # Get channel IDs from slugs first for cache keys
        channels = Channel.objects.using(self.database_connection_name).filter(
            slug__in=keys
        )
        ...
        for channel_slug in keys:
            ...
            if cached_listings is not None:
                shipping_method_channel_listings_by_channel_slug[channel_slug] = (
                    cached_listings
                )
            else:
        ...
        if uncached_slugs:
            shipping_method_channel_listings = (
                ShippingMethodChannelListing.objects.using(
                    self.database_connection_name
                )
                .filter(channel__slug__in=uncached_slugs)
                .annotate(channel_slug=F("channel__slug"))
                .order_by("pk")
            )

            # Group by channel slug and cache
            uncached_map = defaultdict(list)
            for shipping_method_channel_listing in shipping_method_channel_listings:
                channel_slug = shipping_method_channel_listing.channel_slug
                uncached_map[channel_slug].append(shipping_method_channel_listing)
