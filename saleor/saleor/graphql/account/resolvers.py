    ...
    if requester:
        ...
        if requester.has_perms(
            ...
            return (
                ...
                .defer("search_document", "jwt_token_key")
                .first()
            )
...
def resolve_users(info, ids=None, emails=None):
    ...
    if requester.has_perms(
        ...
        qs = models.User.objects.using(connection_name).all().defer(*deferred_fields)
    elif requester.has_perm(AccountPermissions.MANAGE_STAFF):
        qs = models.User.objects.staff().using(connection_name).defer(*deferred_fields)
    elif requester.has_perm(AccountPermissions.MANAGE_USERS):
        qs = models.User.objects.customers().using(connection_name).defer(*deferred_fields)
    elif requester.id:
        ...
        qs = models.User.objects.using(connection_name).filter(id=requester.id).defer(*deferred_fields)
    else:
