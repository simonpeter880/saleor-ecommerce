...
class PromotionQueryset(models.QuerySet["Promotion"]):
    ...
    def flash_sales(self, date=None):
        ...
        return self.filter(
            ...
            Q(stock_limit__isnull=True) | Q(sold_count__lt=models.F("stock_limit"))
        )
...
class Promotion(ModelWithMetadata):
    ...
    is_flash_sale = models.BooleanField(
        default=False,
        help_text="Whether this is a time-limited flash sale",
    )
    flash_sale_label = models.CharField(
        max_length=100,
        blank=True,
        default="",
        help_text="Label to display for the flash sale (e.g., 'Flash Deal', 'Lightning Sale')",
    )
    stock_limit = models.PositiveIntegerField(
        null=True,
        blank=True,
        help_text="Maximum number of items available at flash sale price",
    )
    sold_count = models.PositiveIntegerField(
        default=0,
        help_text="Number of items sold at flash sale price",
    )
