...
export async function executeGraphQL<Result, Variables>(
	...
	if ("errors" in body) {
		...
		// Check if error is due to expired signature/token
		const hasExpiredSignature = body.errors.some(
			(error) =>
				error.message.includes("Signature has expired") || error.message.includes("Invalid token")
		);

		if (hasExpiredSignature) {
			console.warn("Auth token expired - user will need to re-authenticate");
			// Note: Cookies can only be deleted in Server Actions or Route Handlers
			// The Auth SDK will handle token refresh automatically on next request
		}
